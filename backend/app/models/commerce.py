...
class Invoice(Base):
    ...
    paid_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # on-chain watcher fields (populated only for provider='onchain'; see doc 15)
    chain: Mapped[str | None] = mapped_column(Text)
    base_amount: Mapped[float | None] = mapped_column(Numeric(38, 18))
    amount_tolerance: Mapped[float | None] = mapped_column(Numeric(38, 18))
    locked_rate: Mapped[float | None] = mapped_column(Numeric(30, 12))
